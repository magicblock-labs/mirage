...
3. If the user wants to invoke an arbitrary Anchor program (build + sign + send any instruction):
   ...
   - In a non-interactive environment (scripts, CI, agents like Claude Code without a TTY), drive the command entirely through flags instead of the interactive picker:
